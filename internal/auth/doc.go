...
// contract of latere.ai/x/pkg/authz, in Lux's action vocabulary with the
// resource shape of each action, and reads the answer into Lux's codes; a
// Lookup carries the same questions into manifest.Resolve; an OwnerPolicy
// is the decision an installation makes when no authorizer is
// configured. Startup builds all of it from the configuration and refuses
// to start on an issuer it cannot read.
//
