...
func TestObjectOwnersAnswerTheOwnerPolicy(t *testing.T) {
	...
	if _, err := failing.Object(ctx, v1.KindBudget, b.Status.ID); err == nil || !errors.Is(err, errOwnersStoreDown) {
		t.Fatalf("a failing store: %v", err)
	}
}

var errOwnersStoreDown = errors.New("store down")
...
func (failingObjects) Get(context.Context, string, string) (v1.Object, int64, error) {
	return nil, 0, errOwnersStoreDown
}
