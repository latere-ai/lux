...
var knownDrift = map[string]string{
	// GET /v1/requests serves targetDialect "" on a record that chose no
	// target, as spec 009's record table says, while the served document's
	// enum for the member lists the four dialects and no empty value; spec
	// 011's generator owns the enum.
	`$.items[].targetDialect: "" is not in the enum`: "spec 011, the Record schema's targetDialect enum, which lacks the empty value spec 009 gives a record that chose no target",
}
...
var knownDriftCases = []string{"case009RecordHasTheStubsTokens", "case009RefusedRequestHasARecord"}
