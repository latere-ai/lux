...
| a shared catalog | `Provider` and `Model` objects the platform declares as an administrator; callers see them through `provider.read` and `model.use`; a Model the platform stops for everyone takes `spec.disabled: true`, refused `model_disabled` at every door and left out of every model list at once, and set back to `false` it serves the same Keys again | the same objects through the store the platform constructs |
...
| retention and erasure | `LUX_USAGE_RETENTION` keeps each class of caller's usage hourly for as long as its rule says, then as one row per month with the owner or the Key dropped, matched on the Key labels the platform sets; `POST /v1/usage/redact` takes one person's identity out of every usage row, `usage.redact` for the authorizer; `LUX_REQUESTLOG_PARTITION_LABEL` writes the request log under a prefix per value of one Key label, so a bucket lifecycle rule expires each class on its own | the same rules through `metering.RetentionRules` over the platform's own rows |
