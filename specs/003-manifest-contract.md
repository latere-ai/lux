...
| `budgets[]` | []string | absent | yes | one to four Budgets, each by name or `bud_` id, every one of which a call must fit ([037-several-budgets-per-key](.archive/037-several-budgets-per-key.md)); a fifth or a duplicate is `invalid_field`, a missing or refused entry `not_found` at `spec.budgets[i]`; `exclusive_fields` with `budget`; `status.budgets` carries each name and id in order |
